...
class MessageModel(Base):
    ...
    session_id = Column(String, ForeignKey('chatbot_schema.sessions.session_id', ondelete='CASCADE'), nullable=False)
    user_id = Column(String, nullable=False)
...
class MessageRatingModel(Base):
    ...
    message_id = Column(String, ForeignKey('chatbot_schema.messages.message_id', ondelete='CASCADE'), primary_key=True)
...
class ChatDatabase:
            ...
            self.async_engine = create_async_engine(
                ...
                connect_args={
                    ...
                    "server_settings": {
                        ...
                        # Set search_path to use chatbot_schema
                        "search_path": "chatbot_schema"
                    }
                }
            )
    ...
    async def _create_async_schema_if_not_exists(self):
        """Create the chatbot_schema if it doesn't exist (async)"""
        ...
        async with self.async_session() as session:
            try:
                ...
                await session.execute(text("CREATE SCHEMA IF NOT EXISTS chatbot_schema"))
                await session.commit()
                logger.info("Schema 'chatbot_schema' created or already exists (async)")
            except Exception as e:
